...
export default function PayablesSummary() {
  ...
  if (loading) return <div className="p-6 text-gray-500">Loading payables...</div>
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="bg-red-50 border border-red-200 rounded-xl px-6 py-3 text-right">
          <p className="text-xs text-red-400">Total You Owe</p>
      ...
      <div className="overflow-x-auto rounded-xl border">
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              {['Vendor', 'Invoice No', 'Invoice Date', 'Due Date', 
            ...
            {data.length === 0 ? (
              ...
              data.map((item, i) => (
                <tr key={i} className="border-t hover:bg-gray-50 transition">
                  <td className="px-4 py-3 font-medium">{item.vendor}</td>
